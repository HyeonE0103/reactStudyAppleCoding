...
유저가 <input>에 뭔가의 조건을 만족할때 이벤트를 처리하고 싶으면 이벤트핸들러를 부착하면 됨

- onChange: 입력시 코드 실행
- onInput: 입력시 코드 실행
- onMouseOver: 요소에 마우스를 댔을 때 코드 실행
- onScroll: 요소를 스크롤 했을 때 코드 실행
  등 이벤트 핸들러는 매우 다양하고 많으니 자신이 원하는 조건에 맞춰 이벤트 핸들러 부착
